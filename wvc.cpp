...
void
CutyPage::setWebkitVideoCapture(WebkitVideoCapture* capt) {
  mWebkitVideoCapture = capt;
}
...
WebkitVideoCapture::WebkitVideoCapture(CutyPage* page, const QString& output, int delay,int length,int fps, OutputFormat format,
  ...
  mScriptObj = new QObject();
	mFps = fps;
	mCurrentFrame = 1;
	mMaxFrame = length * fps;

  // This is not really nice, but some restructuring work is
  // needed anyway, so this should not be that bad for now.
  mPage->setWebkitVideoCapture(this);
	connect(&timer,SIGNAL(timeout()),this,SLOT(saveSnapshot()));
}

void
WebkitVideoCapture::InitialLayoutCompleted() {
...
void
WebkitVideoCapture::DocumentComplete(bool /*ok*/) {
...
void
WebkitVideoCapture::JavaScriptWindowObjectCleared() {
...
void
WebkitVideoCapture::TryDelayedRender() {
...
void WebkitVideoCapture::captureFinished(){
...
main(int argc, char *argv[]) {
  ...
  for (int ax = 1; ax < argc; ++ax) {
    ...
    } else if(strncmp("--length",s,nlen)==0){
			argLength = (unsigned int)atoi(value);
		} else if(strncmp("--fps",s,nlen)==0){
			argFps=(unsigned int) atoi (value);
		} 
  ...
  if (argInjectScript) {
   ...
   WebkitVideoCapture main(&page, argOut, argDelay,argLength,argFps, format, scriptProp, scriptCode);
